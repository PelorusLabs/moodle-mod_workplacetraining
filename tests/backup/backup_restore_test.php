...
/**
 * Unit tests for workplace training backup and restore.
 *
 * @package    mod_workplacetraining
 * @copyright  Pelorus Labs
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @covers \backup_workplacetraining_activity_task
 * @covers \backup_workplacetraining_activity_structure_step
 * @covers \restore_workplacetraining_activity_task
 * @covers \restore_workplacetraining_activity_structure_step
 */
final class backup_restore_test extends \advanced_testcase {
    ...
    public function test_backup_restore_basic(): void {
        ...
        $workplacetraining = $this->getDataGenerator()->create_module('workplacetraining', [
            'course' => $course->id,
        ...
        $modules = get_coursemodules_in_course('workplacetraining', $newcourse->id);
        $this->assertCount(1, $modules);

        $newmodule = reset($modules);
        $restoredwt = $DB->get_record('workplacetraining', ['id' => $newmodule->instance]);
    ...
    public function test_backup_restore_with_sections(): void {
        ...
        // Create sections.
        $section1 = $this->create_section($workplacetraining->id, 'Section 1', null, 0);

        $this->create_section($workplacetraining->id, 'Section 2', $section1, 1);
        ...
        $sections = $DB->get_records('workplacetraining_sections', ['wtid' => $newmodule->instance], 'position ASC');
    ...
    public function test_backup_restore_with_items(): void {
        ...
        // Create section.
        $sectionid = $this->create_section($workplacetraining->id, 'Section 1', null, 0);
        ...
        $sections = $DB->get_records('workplacetraining_sections', ['wtid' => $newmodule->instance]);
        $section = reset($sections);

        $items = $DB->get_records('workplacetraining_section_items', ['sectionid' => $section->id], 'position ASC');
    ...
    public function test_backup_restore_with_item_configs(): void {
        ...
        // Create configs.
        $DB->insert_record('workplacetraining_item_config', (object) [
            'itemid' => $itemid,
            'name' => 'option1',
            'value' => 'Option 1',
        ]);
        ...
        $items = $DB->get_records('workplacetraining_section_items', ['sectionid' => $section->id]);
        $item = reset($items);

        $configs = $DB->get_records('workplacetraining_item_config', ['itemid' => $item->id], 'name ASC');
    ...
    public function test_backup_restore_with_responses(): void {
        ...
        $responses = $DB->get_records('workplacetraining_responses', ['itemid' => $item->id], 'userid ASC');
    ...
    public function test_backup_restore_without_responses(): void {
        ...
        $responses = $DB->get_records('workplacetraining_responses', ['itemid' => $item->id]);
    ...
    public function test_backup_restore_with_fileupload_files(): void {
        ...
        $cm = get_coursemodule_from_instance('workplacetraining', $workplacetraining->id);
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($context->id, 'mod_workplacetraining', $filearea, false, '', false);
        ...
        $modules = get_coursemodules_in_course('workplacetraining', $newcourse->id);
        $newmodule = reset($modules);
        $newcm = get_coursemodule_from_id('workplacetraining', $newmodule->id);
        ...
        $restoredfiles = $fs->get_area_files($newcontext->id, 'mod_workplacetraining', $newfilearea, false, '', false);
    ...
    public function test_backup_restore_with_multiple_fileupload_items(): void {
        ...
        $files1 = $fs->get_area_files($newcontext->id, 'mod_workplacetraining', $newfilearea1, false, '', false);
        ...
        $files2 = $fs->get_area_files($newcontext->id, 'mod_workplacetraining', $newfilearea2, false, '', false);
    ...
    public function test_backup_restore_complex_hierarchy(): void {
        ...
        $parentsection = $this->create_section($workplacetraining->id, 'Parent Section', null, 0);

        // Create child section 1.
        $childsection1 = $this->create_section($workplacetraining->id, 'Child Section 1', $parentsection, 1);

        // Create child section 2.
        $this->create_section($workplacetraining->id, 'Child Section 2', $parentsection, 2);

        // Create grandchild section.
        $this->create_section($workplacetraining->id, 'Grandchild Section', $childsection1, 3);
