 ...
 */
class backup_workplacetraining_fileupload_files extends backup_execution_step {
    /**
    ...
    protected function define_execution() {
        ...
        $wt = $DB->get_record('workplacetraining', ['id' => $activityid]);
        $cm = get_coursemodule_from_instance('workplacetraining', $wt->id);
        $context = context_module::instance($cm->id);

        // Get all fileupload items in this activity.
        $sql = "SELECT wsi.id, wsi.type
                  FROM {workplacetraining_section_items} wsi
                  JOIN {workplacetraining_sections} ws ON ws.id = wsi.sectionid
        ...
        foreach ($wtitems as $wtitem) {
            $sql = "SELECT r.id, r.itemid, r.userid, r.version FROM {workplacetraining_responses} r WHERE r.itemid = :itemid";
            ...
            foreach ($responses as $response) {
                ...
                $fs = get_file_storage();
                $files = $fs->get_area_files($context->id, 'mod_workplacetraining', $filearea, $response->userid, 'itemid', false);
